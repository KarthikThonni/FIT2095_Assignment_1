...
// --- View engine + static files + body parsing ---
app.set("view engine", "ejs");                        // server-side templates
app.set("views", path.join(__dirname, "views"));      

app.use(express.urlencoded({ extended: true }));      
app.use(express.json());                              

// Serve my own static assets (css, js, images, etc.)
app.use("/public", express.static(path.join(__dirname, "public")));
// Serve Bootstrap locally 
app.use("/bootstrap", express.static(path.join(__dirname, "node_modules/bootstrap/dist")));

// --- Cookie auth (no external sessions) ---
const AUTH_COOKIE = "auth";

// Tiny cookie parser: "key=value; key2=value2" → { key, key2 }
...
// Read the user object stored in the auth cookie
function getAuthFromCookie(req) {
  try {
    const cookies = parseCookies(req);
    if (!cookies[AUTH_COOKIE]) return null;
    // Cookie contains a tiny JSON payload: {userId,email,fullname,role}
    return JSON.parse(cookies[AUTH_COOKIE]);
  } catch {
    // If cookie is malformed, treat as not logged in
    return null;
  }
}

// Write the auth cookie (HttpOnly so JS can't read it on the client)
...
// Make user and studentId available to every EJS view by default
...
// --- Small helper functions I reuse across routes ---
// Turn a textarea into an array (one item per line)
...
// Difference in days (used to flag expiry)
...
// Fallback total value if I’m not aggregating in Mongo (kept for reference)
function sumTotalInventoryValue(items) {
  return items.reduce((acc, it) => acc + (Number(it.cost) || 0), 0);
}

// A1 in-memory ID helper (kept because some A1 routes still reference it)
...
// --- Small validation “enums”  ---
const MEAL_TYPES = new Set(["Breakfast", "Lunch", "Dinner", "Snack"]);
const DIFFICULTIES = new Set(["Easy", "Medium", "Hard"]);
const INV_UNITS = new Set(["pieces","kg","g","liters","ml","cups","tbsp","tsp","dozen"]);
const INV_CATEGORIES = new Set(["Vegetables","Fruits","Meat","Dairy","Grains","Spices","Beverages","Frozen","Canned","Other"]);
const INV_LOCATIONS = new Set(["Fridge","Freezer","Pantry","Counter","Cupboard"]);

// Consistent error redirect to the shared error page
function bad(res, msg) {
  return res.redirect(`/error-${STUDENT_ID}?msg=${encodeURIComponent(msg)}`);
}

// Simple ID and date formats I enforce server-side
const RECIPE_ID_REGEX = /^R-(\d{5})$/;
const INVENTORY_ID_REGEX = /^I-(\d{5})$/;
const ISO_DATE_REGEX = /^\d{4}-\d{2}-\d{2}$/;

// --- Validate a recipe payload ---
// Ensures client-side validation; this guards the database.
function validateRecipeBody(req, res, next) {
  const { title, chef, mealType, cuisineType, prepTime, difficulty, servings, ingredients, instructions, createdDate } = req.body;
  ...
  // date to YYYY-MM-DD inside req.body so downstream code is simpler
...
// --- Validate an inventory payload ---
// prevents bad dates/units and keeps totals meaningful.
function validateInventoryBody(req, res, next) {
  const { userId, ingredientName, quantity, unit, category, purchaseDate, expirationDate, location, cost, createdDate } = req.body;
...
// --- Lightweight fuzzy matching (ingredients ↔ inventory) ---
...
// Handle register submit (unique email, simple userId)
...
// Handle login (set cookie with minimal user info)
...
// Route guard used on recipe pages: only chefs allowed 
function requireChef(req, res, next) {
  const u = getAuthFromCookie(req);
  if (u && u.role === "chef") return next();
  return res.status(403).render("error", { studentId: STUDENT_ID, message: "Recipes are for chefs only." });
}

// --- Recipe features (Mongo) ---
// lean() returns plain objects → faster and simpler for EJS
...
// Fixed list of cuisines for validation and dropdowns
...
// Create a new recipe (ids generated by peeking last recipeId)
app.post(`/api/add-recipe-${STUDENT_ID}`, requireChef, validateRecipeBody, async (req, res) => {
  try {
    ...
    // Normalize textarea inputs → arrays
    const body = { ...req.body };
    ...
    await Recipe.create({
      recipeId,
      userId: auth.userId,      // ownership
      title: body.title.trim(),
      chef: auth.fullname,      // locked from cookie
      ingredients,
      instructions,
...
// Search recipes with regex across a few fields
  ...
  try {
    ...
    if (q) {
      recipes = await Recipe.find({
        $or: [{ title: new RegExp(q, "i") }, { chef: new RegExp(q, "i") }, { cuisineType: new RegExp(q, "i") }],
      }).lean();
    }
    res.render("searchRecipes", { studentId: STUDENT_ID, q, results: recipes });
  } catch (err) {
...
// Scale recipe: simple math using a factor 
app.get(`/scale-recipe-${STUDENT_ID}`, requireChef, async (req, res) => {
  try {
    const allRecipes = await Recipe.find().lean();
    if (!allRecipes.length) {
      return res.render("scaleRecipe", { studentId: STUDENT_ID, allRecipes, selected: null, newServings: "", scaledIngredients: [], noRecipes: true });
    }
    const { recipeId } = req.query;
    const selected = allRecipes.find((r) => r.recipeId === recipeId) || allRecipes[0];

    const rawNew = req.query.newServings;
    const newServings = rawNew && Number(rawNew) > 0 ? String(rawNew) : String((Number(selected.servings) || 1) * 2);
    ...
    res.render("scaleRecipe", { studentId: STUDENT_ID, allRecipes, selected, newServings, scaledIngredients, noRecipes: false });
  } catch (err) {
...
// Delete recipe: two-step UX (confirm page + post)
app.get(`/delete-recipe-${STUDENT_ID}`, requireChef, async (req, res) => {
  try {
    ...
    res.render("deleteRecipe", { studentId: STUDENT_ID, recipes: all, selected, msg: req.query.msg || "" });
  } catch (err) {
...
// Edit recipe: show form then apply update (with server-side validation again)
app.get(`/edit-recipe-${STUDENT_ID}`, requireChef, async (req, res) => {
  try {
    ...
    res.render("editRecipe", { studentId: STUDENT_ID, recipe, mealTypes: [...MEAL_TYPES], difficulties: [...DIFFICULTIES], cuisines: CUISINES, error: req.query.msg || null });
  } catch (err) {
...
app.post(`/api/edit-recipe-${STUDENT_ID}`, requireChef, async (req, res) => {
  try {
    ...
    // Keep author fields from the logged-in user
    const auth = getAuthFromCookie(req);
    const updateDoc = {
      userId: auth.userId,
      chef: auth.fullname,
...
// --- Inventory features (Mongo) ---
// Lists inventory with flags for UI labels, plus a Mongo aggregation for total value.
app.get(`/inventory-${STUDENT_ID}`, async (req, res) => {
  try {
    ...
    const items = docs.map(x => {
      ...
      return { ...x, daysToExpire: dte, isExpiringSoon: dte <= 3, isExpired: dte < 0, isLowStock: isLowStockByUnit(x.quantity, x.unit) };
    });

    // Aggregate in DB (faster and less JS math)
    ...
    res.render("inventory", { studentId: STUDENT_ID, items, totalValue, lowStockCount });
  } catch (err) {
...
app.get(`/add-inventory-${STUDENT_ID}`, (req, res) => {
  res.render("addInventory", { studentId: STUDENT_ID, categories: [...INV_CATEGORIES], locations: [...INV_LOCATIONS], units: [...INV_UNITS] });
});

// Middleware: copy userId from cookie into body (so form can't spoof)
...
// Create a new inventory item (sequential readable id, server validation)
app.post(`/api/add-inventory-${STUDENT_ID}`, injectUserIdFromCookie, validateInventoryBody, async (req, res) => {
  try {
    const b = req.body;
...
// Delete inventory item by its friendly id
...
// Edit inventory: show form then update (uses same validator to keep rules consistent)
...
app.post(`/api/edit-inventory-${STUDENT_ID}`, injectUserIdFromCookie, validateInventoryBody, async (req, res) => {
  try {
    ...
    if (!inventoryId || !INVENTORY_ID_REGEX.test(inventoryId)) return bad(res, "Invalid inventoryId.");

    const userId = getAuthFromCookie(req)?.userId;
    const updated = await Inventory.findOneAndUpdate(
      { inventoryId },
      { $set: { userId, ingredientName: ingredientName.trim(), quantity: Number(quantity), unit, category, purchaseDate, expirationDate, location, cost: Number(cost), createdDate } },
      { runValidators: true }
    );
...
// --- Pantry integration (check + suggest) ---
//Readable matching 
app.get(`/check-ingredients-${STUDENT_ID}`, async (req, res) => {
  try {
    const allRecipes = await Recipe.find({}).lean();
    const selected = allRecipes.find(r => r.recipeId === String(req.query.recipeId || "")) || allRecipes[0] || null;
    ...
    if (selected) {
      ...
      const pct = selected.ingredients?.length ? Math.round((have.length / selected.ingredients.length) * 100) : 0;
...
// --- Misc / shared routes ---
// Root goes to the role-aware home
app.get("/", (req, res) => {
  res.redirect(`/home-${STUDENT_ID}`);
});

// Home dashboard: pulls counts in parallel 
app.get(`/home-${STUDENT_ID}`, async (req, res) => {
  ...
  try {
    ...
    res.render("index", {
      studentId: STUDENT_ID,
      stats: { totalUsers, recipeCount, inventoryCount, cuisineTypes: cuisines.length, totalInventoryValue },
    });
  } catch (err) {
    console.error("Home stats error:", err);
    res.render("error", { studentId: STUDENT_ID, message: "Failed to load dashboard stats." });
  }
});

// Quick route listing for debugging (helps me check paths fast)
...
app.listen(PORT, () => {
  console.log(`CloudKitchen Pro running at http://localhost:${PORT}/home-${STUDENT_ID}`);
});
