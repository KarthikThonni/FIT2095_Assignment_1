    ...
    <div class="container mt-4" style="max-width:520px">
      ...
      <form (ngSubmit)="onSubmit()" #f="ngForm" novalidate>
        ...
        <div class="mb-3">
          ...
          <small class="text-muted">8+ chars, include uppercase, lowercase, number, special.</small>
        </div>
        <div class="mb-3">
...
export class RegisterComponent {
  email=''; password=''; fullname=''; role='chef'; phone=''; error=''; 
  passwordPattern = PASSWORD_COMPLEXITY.source;
  constructor(private auth: AuthService, private router: Router) {}
  onSubmit() {
    this.error = '';
    this.auth.register({ email: this.email, password: this.password, fullname: this.fullname, role: this.role, phone: this.phone })
      .subscribe(ok => { if (ok) this.router.navigateByUrl('/login'); else this.error = 'Registration failed.'; });
  }
}
